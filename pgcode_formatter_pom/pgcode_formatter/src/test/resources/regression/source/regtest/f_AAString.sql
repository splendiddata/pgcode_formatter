...
BEGIN
    ...
    country2012.val := 'Ours';
        INSERT INTO tbl_country2012_l1 (
            arrIx,
            val)
        VALUES (
            'Taylor Swift',
    ...
    while ( coalesce(l_idx, '') != '' )
        loop
        SELECT tt.val INTO STRICT country2012
        ...
        SELECT tt.val INTO STRICT songlist_1
    ...
    country2012.val := 'Begin Again';
        ...
        ON CONFLICT(arrIx)
          DO UPDATE SET
                val = country2012.val;
    SELECT tt.val INTO STRICT country2012_2
      FROM tbl_country2012_l1 tt
    ...
    --  EXISTs test
    --
    if (SELECT (CASE count(*) WHEN 0 then FALSE else TRUE END) FROM tbl_country2012_l1 WHERE arrIx = v_key) THEN
    ...
    BEGIN
        SELECT tt.val INTO STRICT songlist_5
          FROM tbl_songlist_l2 tt
    ...
    BEGIN
        ...
        country2012.val := 'American Soldier'; -- oldie
            ...
            ON CONFLICT(arrIx)
              DO UPDATE SET
                    val = country2012.val;
        SELECT tt.val INTO STRICT country2012_6
          FROM tbl_country2012_l1 tt
    ...
    BEGIN
        ...
        country2012.val := upper ( 'American Soldier' ); -- oldie
            ...
            ON CONFLICT(arrIx)
              DO UPDATE SET
                    val = country2012.val;
        SELECT tt.val INTO STRICT country2012_7
          FROM tbl_country2012_l1 tt
    ...
    country2012.val := v_val;
        ...
        ON CONFLICT(arrIx)
          DO UPDATE SET
                val = country2012.val;
    v_key := 'Toby2';
    v_keyPrior := (SELECT last_value(arrIx) over (order by arrIx desc) FROM tbl_country2012_l1 WHERE arrIx < v_key limit 1);
    RAISE NOTICE 'f_AAString.....| key prior to Toby2 is %', v_keyPrior;
    SELECT tt.val INTO STRICT country2012_9 FROM tbl_country2012_l1 tt WHERE arrIx = (SELECT last_value(arrIx) over (order by arrIx desc)
      FROM tbl_country2012_l1
     WHERE arrIx < v_key limit 1);   -- v_valPrior := country2012(countr ...
    ...
    SELECT tt.val INTO STRICT country2012_10 FROM tbl_country2012_l1 tt WHERE arrIx = (SELECT arrIx
      FROM tbl_country2012_l1
     WHERE arrIx > v_key ORDER BY arrIx LIMIT 1);   -- v_valNext := country2012(country ...
