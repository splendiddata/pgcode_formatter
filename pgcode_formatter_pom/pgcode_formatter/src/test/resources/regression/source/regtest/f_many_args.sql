...
BEGIN
    if ( coalesce(argum_dc_zap::text, '') = '') and
        ( coalesce(argum_dc_bin::text, '') = '') and
        ( coalesce(argum_dv_zap::text, '') = '') and
        ( coalesce(argum_dv_bin::text, '') = '') and
        ( coalesce(argum_da_zap::text, '') = '') and
        ( coalesce(argum_da_bin::text, '') = '') and
        ( coalesce(argum_de_zap::text, '') = '') and
        ( coalesce(argum_de_bin::text, '') = '') and
        ( coalesce(argum_as_dest::text, '') = '') and
        ( coalesce(argum_tc_dest::text, '') = '') then
        ...
        ln_count := some_schema.F_ALSO_MANY(
                      ...
                      argum_tc_dest )
        -
        some_schema.F_STILL_MUCH(  argum_da_bin ,
                                  argum_de_zap ,
                                  argum_de_bin ,
                                  argum_as_orig ,
                                  argum_as_dest ,
