/* comment */
CREATE OR REPLACE FUNCTION "increment"(i integer, out b numeric(20)) AS $function$
        BEGIN
                a := CASE
                     WHEN b THEN true ELSE false
                     END;
                RETURN 'test';
        END;
$function$ LANGUAGE plpgsql immutable security definer not leakproof;
