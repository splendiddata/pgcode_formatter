...
BEGIN
    IF (coalesce(argum_dc_zap::TEXT, '') = '') AND (coalesce(argum_dc_bin::TEXT, '') = '') AND (coalesce(argum_dv_zap::TEXT, '') = '') AND
        
            ( coalesce(argum_dv_bin::TEXT, '') = '' ) AND (coalesce(argum_da_zap::TEXT, '') = '') AND (coalesce(argum_da_bin::TEXT, '') = '') AND
        
            ( coalesce(argum_de_zap::TEXT, '') = '' ) AND (coalesce(argum_de_bin::TEXT, '') = '') AND (coalesce(argum_as_dest::TEXT, '') = '') AND
        
            ( coalesce(argum_tc_dest::TEXT, '') = '' )
    THEN
    ...
    ELSE
        ln_count := some_schema.F_ALSO_MANY( common_node, argum_dc_zap, argum_dc_bin, argum_dv_zap, argum_dv_bin
            ...
            some_schema.F_STILL_MUCH( argum_da_bin, argum_de_zap, argum_de_bin, argum_as_orig, argum_as_dest
