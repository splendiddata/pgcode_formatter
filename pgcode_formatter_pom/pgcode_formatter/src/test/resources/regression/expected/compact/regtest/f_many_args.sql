CREATE OR REPLACE FUNCTION some_schema.f_many_args(
	common_node NUMERIC, argum_dc_zap VARCHAR,
	argum_dc_bin VARCHAR, argum_dv_zap VARCHAR,
	argum_dv_bin VARCHAR, argum_da_zap VARCHAR,
	argum_da_bin VARCHAR, argum_de_zap VARCHAR,
	argum_de_bin VARCHAR, argum_as_orig VARCHAR,
	argum_as_dest VARCHAR, argum_tc_orig VARCHAR,
...
BEGIN
	IF (coalesce(argum_dc_zap::TEXT, '') = '') AND (coalesce(argum_dc_bin::TEXT, '') = '') AND
	  ...
	  (coalesce(argum_as_dest::TEXT, '') = '') AND (coalesce(argum_tc_dest::TEXT, '') = '') THEN
	...
	ELSE
		ln_count := some_schema.F_ALSO_MANY(
				...
				argum_tc_dest ) - some_schema.F_STILL_MUCH(
				argum_da_bin, argum_de_zap, argum_de_bin,
				argum_as_orig, argum_as_dest, argum_tc_orig,
				argum_tc_dest );
	END IF;
