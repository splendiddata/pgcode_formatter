 ...
 * <p>
 * The output files are the target/test/completeness/out directory.
 * </p>
 *
 * @author Splendid Data Product Development B.V.
 * @since 0.0.1
 */
public class OutputCompletenessTest {
    private static final Logger log = LogManager.getLogger(OutputCompletenessTest.class);
    ...
    static void beforeAll() throws IOException {
        ...
        outputBaseDirectory = Paths.get(projectDirectory.toString(), "target/test/completeness/out").toAbsolutePath();
     ...
     * directories
     *
     * @return Stream&lt;Case&gt; with for each source - config combination a test dase
     */
    ...
    void testSqlFileWithConfig(TestCase testCase) throws IOException {
        ...
        try {
            ...
            input = replace(input).toLowerCase();
            ...
            if (!output.equalsIgnoreCase(input)) {
                ...
                Path parent = testCase.getOutputFile().getParent();
                Path fileName = testCase.getOutputFile().getFileName();
                Path inputPath = Paths.get(parent.toString() + "/input/" + fileName);
                Path outputPath = Paths.get(parent.toString() + "/output/" + fileName);
                Path formattedOutputPath = testCase.getOutputFile();

                filesInError.append("\n    ").append(outputPath);
                Files.createDirectories(outputPath.getParent());
                Files.createDirectories(inputPath.getParent());

                try (BufferedWriter inputWriter = Files.newBufferedWriter(inputPath, StandardCharsets.UTF_8);
                        BufferedWriter outputWriter = Files.newBufferedWriter(outputPath, StandardCharsets.UTF_8)) {
                    inputWriter.append(input.replaceAll("\\s", "\n"));
    ...
    private Node maskFunctionBody(Node stmt, List<String> parseErrors) {
        ...
        case "sql":
            for (DefElem defElem : defElemList) {
                if ("as".equals(defElem.defname)) {
                    SqlParser parser = new SqlParser(new StringReader(((List<Value>) defElem.arg).get(0).val.str),
    ...
    private static final class PgParserErrorReporter implements SqlParserErrorReporter {
        ...
        /**
         * @see SqlParserErrorReporter#reportError(SqlParserErrorData) Appends the error text to the errorsFound list
         * @param err
