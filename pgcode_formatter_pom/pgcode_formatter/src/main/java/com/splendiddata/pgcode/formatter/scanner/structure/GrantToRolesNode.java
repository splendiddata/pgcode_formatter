...
/**
 * The TO role specification in a GRANT command.
 */
public class GrantToRolesNode extends SrcNode {
    ...
    private SrcNode toClause;

    public GrantToRolesNode(ScanResult scanResult) {
        super(ScanResultType.SELECT_STATEMENT, scanResult);
        identifyTargetListElements(scanResult);

    }

    /**
     * Identifies the constituent parts of the rol specification
     */
    private void identifyTargetListElements(ScanResult argumentsStart) {
        ...
        if ("to".equalsIgnoreCase(node.getText().toLowerCase())) {
            toClause = new IdentifierNode(node);
            toClause.setNext(null);
            node = node.getNext();
        } else {
            return;
        }
        ...
        for (;;) {
            ...
            assert argumentPart != null : "The argumentPart must not be null here";
    ...
    public String toString() {
        ...
        if (toClause != null) {
            result.append(toClause.toString());
        }
    ...
    public RenderResult beautify(FormatContext formatContext, RenderMultiLines parentResult, FormatConfiguration config) {
        ...
        RenderResult renderResult = toClause.beautify(formatContext, result, config);
