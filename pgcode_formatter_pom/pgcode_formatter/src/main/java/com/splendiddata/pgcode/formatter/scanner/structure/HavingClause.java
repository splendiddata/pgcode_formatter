 ...
 * @since 0.0.1
 */
public class HavingClause extends ClauseThatStartsWithMajorKeyword {
    /**
     * Constructor
     *
     * @param startNode
     *            The ScanResult that reads HAVING
     */
    public HavingClause(ScanResult startNode) {
        super(ScanResultType.INTERPRETED, new IdentifierNode(startNode));
        assert "having".equalsIgnoreCase(startNode.toString());

        /*
         * argument list
         */
        ScanResult priorNode = getStartScanResult().locatePriorToNextInterpretable();
        ScanResult currentNode = CommaSeparatedList.withArbitraryEnd(priorNode.getNext(),
