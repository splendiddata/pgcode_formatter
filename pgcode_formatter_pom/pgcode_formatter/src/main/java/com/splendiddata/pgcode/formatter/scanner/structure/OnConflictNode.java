...
public class OnConflictNode extends SrcNode implements WantsNewlineBefore {
    ...
    public RenderMultiLines beautify(FormatContext formatContext, RenderMultiLines parentResult, FormatConfiguration config) {
        ...
        if (!config.getQueryConfig().isMajorKeywordsOnSeparateLine().booleanValue()) {
            ...
            for (ScanResult node = getStartScanResult(); node != null; node = node.getNext()) {
                result.addRenderResult(
                        node.beautify(formatContext, result, config),
                        formatContext);
            }
         ...
         */
        RenderMultiLines result = new RenderMultiLines(this, formatContext);
        FormatContext contentContext = new FormatContext(config, formatContext).setAvailableWidth(availableWidth - standardIndent);
        ScanResult node = getStartScanResult();
        result.addRenderResult(node.beautify(formatContext, result, config), formatContext);
        if (config.getQueryConfig().isMajorKeywordsOnSeparateLine().booleanValue()) {
            while (node != null && !(node.is(ScanResultType.IDENTIFIER) && "conflict".equalsIgnoreCase(node.toString()))) {
                    node = node.getNext();
        ...
        for (node = node.getNext(); node != null; node = node.getNext()) {
            ...
            if (result.getPosition() > standardIndent && result.getPosition() + itemResult.getWidth() > availableWidth) {
                result.addLine();
                result.addRenderResult(node.beautify(contentContext, result, config), formatContext);
            } else {
                result.setIndent(result.getPosition());
                result.addRenderResult(itemResult, formatContext);
                result.setIndent(standardIndent);
            }
        }
